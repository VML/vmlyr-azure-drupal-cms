...
class LibraryDiscoveryParser {
   ...
   *
   * @todo Remove BC layer in Drupal 9.0.0 https://www.drupal.org/node/2852314.
   *
   * @var array
   */
  ...
  protected function setOverrideValue(array $overrides, $type, $library_name, $extension, $theme_path) {
    foreach ($overrides as $original => $replacement) {
      ...
      if (isset($this->originalLibraryPaths[$original])) {
        ...
        // asset was keyed.
        @trigger_error('Overriding a library asset using an overridden path as key is deprecated, please use the original path as key.', E_USER_DEPRECATED);
