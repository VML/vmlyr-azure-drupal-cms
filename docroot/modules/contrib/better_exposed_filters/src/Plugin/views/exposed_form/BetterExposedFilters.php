...
Title Desc|Z -> A</pre> Leave the replacement text blank to remove an option altogether. The order the options appear will be changed to match the order of options in this field.'),
    ...
    foreach ($this->view->display_handler->getHandlers('filter') as $label => $filter) {
      ...
      if (is_a($filter, 'Drupal\taxonomy\Plugin\views\filter\TaxonomyIndexTid')) {
        ...
        // widget.
        if (isset($this->displayHandler->handlers['filter'][$label]->options['type']) && $this->displayHandler->handlers['filter'][$label]->options['type'] != 'select') {
          $bef_standard = FALSE;
        }
        elseif (isset($this->displayHandler->handlers['filter'][$label]->options['widget']) && $this->displayHandler->handlers['filter'][$label]->options['widget'] != 'select') {
          $bef_standard = FALSE;
        }
      ...
      if ((isset($filter_form[$filter_id]['#type']) &&
          in_array($filter_form[$filter_id]['#type'], $supported_types)) ||
        (isset($filter_form[$filter_id]['value']) &&
          (in_array($filter_form[$filter_id]['value']['#type'], $supported_types)))) {
